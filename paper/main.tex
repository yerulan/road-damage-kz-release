...
\dataavailability{Code, manifests, annotations, generated reports, and reconstruction instructions are made available in a public repository. The publishable Kazakhstan benchmark is built from open-license sources with recorded attribution and license metadata; privacy-safe derivatives may be released where permitted by the source license and privacy review. Image provenance and source URLs are recorded in the manifest so that users can reconstruct local caches subject to each source license. RDD data must be downloaded from the official RoadDamageDetector/RDD2022 source according to its terms.}

\acknowledgments{The authors thank the maintainers of the RoadDamageDetector/RDD datasets and the open-license image communities whose metadata enables reproducible source auditing.}
